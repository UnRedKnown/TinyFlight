...
class PerlinNoise:
    def __init__(self):
        ...
        for i in range(256):
            self.p[256 + i] = self.p[i] = permutation[i]
    ...
    def noise(self, x, y):
        ...
        BA = self.p[B]
        BB = self.p[B + 1]
        return self.lerp(v, self.lerp(u, self.grad(self.p[AA], x, y), self.grad(self.p[BA], x - 1, y)),
                            self.lerp(u, self.grad(self.p[AB], x, y - 1), self.grad(self.p[BB], x - 1, y - 1)))

    def generate_noise(self, width, height, pos, scale):
        noise_array = []
        ...
        for y in range(height):
            ...
            for x in range(width):
                noise = self.noise((x + offset_x) / scale, (y + offset_y) / scale)
                row.append(noise)
